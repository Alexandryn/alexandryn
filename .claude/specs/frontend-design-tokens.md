...
- **FR-2** Extraction is a documented, repeatable process, not a one-time
  ...
  declarations, maps each to FR-1's taxonomy, and writes Tailwind theme
  config — run again whenever the design reference changes (CLAUDE.md's
  own "re-pull before trusting this section is current" discipline
  applied to tokens specifically), not treated as a single import
  frozen at phase 04's start.
...
- **Design reference (`.dc.html` CSS custom properties) → Tailwind theme
  config**: the one extraction contract this spec fixes (FR-1/FR-2).
