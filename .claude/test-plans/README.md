...
cadence. Start from [`../templates/test-plan.md`](../templates/test-plan.md).

**Form, for a phase sliced into tiers.** When a phase's implementation is
broken into tiers (phase 04's Tiers 0–6 are the precedent), the test plan
for the specs it implements is: **(a)** each spec's own **Test strategy**
and **Acceptance criteria** sections, written at spec approval before any
implementation; **(b)** the per-tier RED-step breakdown — per-task
acceptance criteria in `tasks/plan-<phase>.md`'s Task list and each
tier's **Checkpoint** pass conditions, elaborated in the
`tasks/plan-<phase>-<tier>.md` / `tasks/todo-<phase>-<tier>.md` docs where
a tier is large enough to warrant one. A separate
`.claude/test-plans/<spec>.md` file is not also required. A phase
implemented in one pass (phase 03) gets the standalone files below.
Either way, a phase does not close until its Exit criteria confirm a test
plan exists in one of these two forms (ADR 0016, Option A's enforcement
line).
...
| Phase 04 — `frontend-tooling`, `frontend-design-tokens`, `frontend-component-primitives`, `frontend-generated-covers`, `frontend-shell-and-routing`, `frontend-accessibility` | same                                                                      | Test plan is each spec's own Test strategy / Acceptance criteria sections plus the per-task acceptance criteria and Checkpoint conditions in [`tasks/plan-phase04.md`](../../tasks/plan-phase04.md) and `tasks/plan-p04-tier{2..6}.md` (ADR 0016 form; phase 04 Exit criteria confirm this) |
