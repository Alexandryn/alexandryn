...
4. Write the message in **Conventional Commits format** (https://www.conventionalcommits.org/en/v1.0.0/): `<type>(<optional scope>): <description>`.
   - Types, matching this repo's own branch prefixes (`CONTRIBUTING.md`) plus the subset of the [react-spectrum PR naming convention](https://github.com/adobe/react-spectrum/wiki/Pull-Request-Naming-Guide) that's actually relevant here: `feat` (new capability), `fix` (bug fix), `refactor` (behavior-preserving restructuring), `spec` (adding, reviewing, or revising anything under `.claude/specs/`, `.claude/decisions/`, `.claude/reviews/`, `.claude/audits/`, or `.claude/roadmap/`), `docs` (README, CONTRIBUTING, CLAUDE.md, or other project documentation — not `.claude/`'s engineering-memory documents, which are `spec`), `build` (tooling, dependencies), `ci` (GitHub Actions / CI configuration, once it exists), `chore` (repo housekeeping that isn't build-related), `test` (test-only changes), `bump` (dependency version increases specifically — use `build` for other tooling changes), `revert` (reverting a previous commit or PR).
