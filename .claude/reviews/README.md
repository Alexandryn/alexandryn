...
| [0038](0038-phase11-cross-spec-review.md) | Phase 11 — three reader specs, cross-spec, two independent agents; plus cross-phase amendments to `frontend-library-screens.md` and `backend-library-api.md` | 2026-08-15 | Needs rework at review time (7 Blocking, 3 confirmed independently by both passes, now fixed), genuinely independent (not self-reviewed) |
...
| [0043](0043-spec-backend-persistence-container-topology.md) | Spec — `backend-persistence.md`, amendment (container topology, ADR 0015) | 2026-08-16 | Approved with changes (finding fixed before this review), self-reviewed — needs independent read |
...
| [0048](0048-phase02-correctness-review.md) | Phase 02 — all five domain specs, correctness pass against constitution, ADRs 0009/0010, `go-backend-conventions/SKILL.md` and `backend-persistence.md` | 2026-08-19 | Approved with changes (four correctness defects and two architectural gaps, all addressed or scheduled), self-reviewed — needs independent read |
| [0049](0049-real-world-edge-case-conformity-review.md) | Real-world edge-case conformity pass — `domain-bibliographic.md`, `domain-source.md`, `domain-reading.md`, `backend-source-adapter.md`, `backend-file-extractors.md`, `backend-import-pipeline.md`, phase 14 roadmap outline | 2026-08-20 | Approved with changes (six gaps fixed, two recorded as open questions, not solved), self-reviewed — needs independent read |
| [0050](0050-phase13-spec-package-and-phase12-authz-review.md) | Phase 13 — ADR 0028 + four network-access specs, cross-spec, two independent agents (quality + security/STRIDE); plus **two confirmed phase-12 authorization defects found during the review** (reading API not user/library-scoped; access-token `typ` not checked) and a correction to audit `0012` | 2026-09-02 | Needs rework — 3 Blocking + 8 Major on the spec package; 2 High phase-12 code defects blocking phase 13; genuinely independent (two agents, no shared context). Resolution tracked in `roadmap/13-network-access/implementation-plan.md` Gate 1.5 |
