...
| | |
|---|---|
| **Status** | `APPROVED` (independent review, findings fixed, maintainer signed off 2026-08-15) — amended 2026-09-01 to realign FR-2/FR-3, the API contract, failure modes, and acceptance criteria with `domain-reading.md`'s own 2026-09-01 amendment (`0048` finding 1). **Amended again 2026-09-02 (`DRAFT`, pending re-confirmation) for phase-13 review `0050` AUDIT-0012-C1:** new FR-9 makes per-user + per-library scoping a hard, handler-level, CI-guarded requirement on every reading endpoint and `export` — the phase-13 **close gate**, because the shipped phase-12 implementation left the whole surface a horizontal IDOR. Non-goals updated (per-user authz is now required; the library-visible "finished / leaderboard" view is deferred to phase 15). Maintainer re-confirmation pending. |
| **Phase** | `11-reader` |
...
- **FR-9** *(added 2026-09-02 for phase-13 review `0050` AUDIT-0012-C1 — the phase-13 close gate.)* Every endpoint in this spec, plus `GET /api/v1/reading/export`, MUST be scoped to **both** the authenticated user and the validated active library, enforced **in the query the wired handler runs**:
  ...
  - **By-id operations** (`GET`/`PATCH`/`DELETE` a bookmark or highlight by its own id) MUST carry `AND user_id = $n` in the SQL, so a row cannot be read, modified, or deleted by knowing its id alone. A **PATCH MUST update only the mutable fields** (a highlight's note/category) with a scoped `UPDATE … WHERE id = $ AND user_id = $` — it MUST NOT re-`SaveForUser` the whole row, which would relocate it into whatever library the request's `X-Library-Id` names (PR #78 review, finding 1).
  - **Writes are gated on library ownership, matching the read path.** Before a bookmark/highlight/progress write is scoped to the active library, the handler MUST confirm the target edition (or, for progress, the work) is owned in that library — `LibraryEntryRepository.EditionInLibrary` / `WorkInLibrary`. A write against an edition or work outside the caller's active library is a `404` (same body as "does not exist" — no cross-library oracle), not a stored row referencing a foreign namespace (PR #78 review, finding 3).
