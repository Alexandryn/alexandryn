...
| | |
|---|---|
| **Status** | `APPROVED` (maintainer read 2026-08-19; amended same day for `0048` findings 3 and 9 — FR-4 now states that a merge moves nothing and that resolve-through covers a `Work`'s relations transitively, making undo exactly the identity; FR-8 forbids cycles by reachability rather than self-reference, and FR-8/FR-9 are checked over the merge-resolved graph by a domain service per [ADR 0020](../decisions/0020-graph-invariants-in-domain-services.md), which also closes the containment/merge open question. Amended 2026-08-20 for `0049` finding 5 — FR-6 now rejects an empty or whitespace-only string the same as one exceeding the length bound; needs re-confirmation) |
| **Phase** | `02-domain` |
| **Author** | Claude (Sonnet 5), for review by Luann Moreira |
| **Created** | 2026-08-14 |
| **Last updated** | 2026-08-14 |
| **Supersedes** | — |
| **Reviewed in** | `0016` (self) + `0021` (two independent agents, cross-spec) — both Approved with changes, all findings fixed; maintainer read and approved 2026-08-19; correctness pass `0048` |
...
- **FR-4** The domain MUST NOT compute whether two Works are "the same
  ...
  **What a merge moves: nothing.** Recording a merge writes exactly one
  ...
  and nothing in this model does (`0048`,
  finding 3).
...
- **FR-6** Every free-form string field on `Work` or `Edition` (title,
  ...
  **A present field's value MUST also fail this validation when it is
  ...
  caller to discover the difference downstream (`0049`,
  finding 5).
...
- ~~**Interaction between FR-9's containment graph and FR-4/FR-8's merge
  graph**~~ — **resolved 2026-08-19** (`0048`).
...
- `docs/roadmap/02-domain/README.md` — this phase's objective, risk
  table, exit criteria
