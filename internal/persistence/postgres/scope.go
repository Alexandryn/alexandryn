...
// argument. This replaces the earlier COALESCE-wrapped comparison against
// an empty string, which was not sargable and treated a NULL owner as a
// match for an empty scope (#118). Reading handlers reject an empty user
// or library before they reach a scoped repository method; the IS NULL
// branch exists only for the pre-tenancy bare Find/Save methods still
// used by tests.
