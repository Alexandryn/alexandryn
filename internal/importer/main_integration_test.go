...
func TestMain(m *testing.M) {
	os.Exit(testutil.IntegrationTestMain(
		os.LookupEnv,
		testutil.WithPackageDatabase("importer", os.Getenv, os.Setenv, os.Stderr, m.Run),
		os.Stderr,
	))
}
...
func mustExecPool(t *testing.T, pool *pgxpool.Pool, sqlQuery string, args ...any) {
	t.Helper()
	if _, err := pool.Exec(context.Background(), sqlQuery, args...); err != nil {
		t.Fatalf("Exec(%q): %v", sqlQuery, err)
	}
}
