...
- [ ] D1 — audit file: `.claude/audits/0004-phase04-frontend-foundation.md`, findings `A-0004-NN`
...
- [ ] T8 — Fix any Critical/High finding — one commit per fix, a regression test per fix. If none: record "no Critical/High findings" explicitly in the audit verdict.
...
- [ ] T10 — Walk `.claude/roadmap/04-frontend-foundation/README.md` Exit criteria item-by-item, each citing real evidence (test name / CI step / file path / PR number — per the plan's evidence table). Write the walk into the README as a Closure section; update the README header (`Status`, `Opened`, `Closed`). Commit (doc).
- [ ] T11 — Move all six `frontend-*.md` spec statuses `APPROVED → IMPLEMENTED → VERIFIED`. Per spec: re-read its Acceptance criteria, cite evidence each is met, then update the spec's status line **and** `.claude/specs/README.md`'s status column. Gated on T10. Commit (doc).
- [ ] T12 — Documentation sweep: `web/README.md`, root pointer files (only if they carry a phase-status line), `tasks/todo-phase04.md` (Tier 6 + P4-G done), `.claude/roadmap/README.md` index, CLAUDE.md "Where the task list lives" (verify not stale). Commit (doc).
- [ ] T13 — Full regression (plan §"Full regression"). Kill stray vite on 5174–5176 first. All green.
