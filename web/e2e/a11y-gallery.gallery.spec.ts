...
// frontend-accessibility.md FR-5 / Acceptance criterion 5: the
// per-primitive `motion-reduce:` rules (Tier 2) actually compose. The
// gallery renders a spinning Spinner and a pulsing Skeleton permanently
// and has no service worker, so emulateMedia is deterministic. Read the
// computed `animation-name` rather than getAnimations() — it is
// independent of the element's size and visibility in the harness layout.
